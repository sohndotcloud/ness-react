...
export default function Sidebar() {
    ...
    return (
        <div>
            ...
            {/* Panel */}
            <div
                className={`absolute top-0 left-0 w-[280px] h-[calc(100vh-72px)] bg-slate-950/90 backdrop-blur-md border-r border-cyan-500/10 z-10 shadow-2xl transition-transform duration-300 ease-in-out ${
                    sideMenu ? "translate-x-0" : "-translate-x-full"
                }`}
            >
                <div className="pt-20 px-4">
                    <div className="text-[10px] uppercase tracking-[0.15em] text-cyan-400/70 font-mono px-4 mb-2">
                        Navigate
                    </div>
                    ...
                    <ul className="list-none">
                        {NAV_ITEMS.map((item) => (
                            <li key={item.label}>
                                <a
                                href={item.href}
                                className="flex items-center px-4 py-3 rounded-md text-sm text-slate-200 hover:bg-cyan-500/10 hover:text-cyan-300 transition-colors duration-150"
                                >
                                {item.label}
                            </a>
                            </li>
