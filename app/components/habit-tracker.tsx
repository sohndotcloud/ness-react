  ...
  return (
    <div className="flex items-center gap-2 rounded-md border-2 border-cyan-400 bg-cyan-500/10 p-2.5 focus-within:outline focus-within:outline-2 focus-within:outline-cyan-400">
      ...
      <button
        type="button"
        onClick={submit}
        className="shrink-0 rounded-md bg-cyan-500 px-3 py-1.5 text-sm font-medium text-white hover:bg-cyan-600 dark:hover:bg-cyan-400 transition-colors duration-150 focus-visible:outline focus-visible:outline-2 focus-visible:outline-cyan-400"
      >
        Add
      </button>
      <button
...
export default function HabitTracker() {
  ...
  function toggleDay(habitId: number, dayIndex: number): void {
    setHabits((prev) =>
      prev.map((h) => {
  ...
  function addHabit(name: string): void {
    setHabits((prev) => [
      ...prev,
      { id: nextId.current++, name, streak: 0, done: [0, 0, 0, 0, 0, 0, 0] },
    ]);
  }
